...
test_network_info() {
    ...
    if [ $? -eq 0 ] && echo "$response" | grep -q '"result"'; then
        # Try to extract build/version info from getinfo
...
test_peer_info() {
    ...
    if [ $? -eq 0 ]; then
        # In regtest with no external peers, this should return empty array or result
        # Just check that the RPC call succeeds
